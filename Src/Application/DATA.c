...
#include <stdlib.h>
	...
	void ReadDataFromFlash(uint16_t page, float32_t *data, uint16_t len)
	{
		...
		for (int i = 0; i < len; i++)
		{
			uint16_t audioSample = ((uint16_t)(pcmData[2 * i])) |
								   ((uint16_t)(pcmData[2 * i + 1]) << 8);
	...
	void WriteDataToFlash(uint16_t page, float32_t *data, uint16_t len)
	{
		...
		for (int i = 0; i < len; i++)
		{
			int32_t audioSample = (int32_t)(data[i] * 32768.0f) + 32768;
			if (audioSample > 65535)	audioSample = 65535;
			if (audioSample < 0)		audioSample = 0;
